import heapq
import copy
from python_actr import logger
from python_actr.errors import SchedulerException
...
class Event:
    ...
    def __repr__(self):
        return f'<{self.name} {id(self.func)} {self.time:.3f}>'
...
class Scheduler:
    ...
    def add(self, func, delay=0,
            args=[], keys={}, priority=0, thread_safe=False):
        if thread_safe:
            self.to_be_added.append((func, delay, args, keys, priority))
        else:
            ev = Event(func, self.time+delay, args=args,
    ...
    def handle_result(self, result, event):
        if isinstance(result, (int, float)):
            ...
            raise SchedulerException(f"Incorrect 'yield': {result}")
